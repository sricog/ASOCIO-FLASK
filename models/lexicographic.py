...
def solve_work_assignment_lexicographic(data_file, verbose=True, time_limit=300, gap_tolerance=0.01):
    ...
    # Reutilizar las variables existentes (no crear nuevas)
    # Las variables X, Y, Z_var ya existen y se reutilizan
    
    # Variables adicionales para tracking de zonas
    W = {}
    for i in E:
        for j in T:
            for z in Z:
    ...
    # Objetivo 5: Cohesión de equipos en la misma zona (peso menor)
        ...
        for j in T:
            for z in Z:
                for i in E_g[g]:
                    ...
                    model2 += cohesion_vars[g, j, z, i] <= W[i, z, j], f"Cohesion_1_{g}_{j}_{z}_{i}"
                    model2 += cohesion_vars[g, j, z, i] <= (team_in_zone - 1) / len(E_g[g]), f"Cohesion_2_{g}_{j}_{z}_{i}"

    # Tracking de zonas
    for i in E:
        for j in T:
            for z in Z:
                model2 += pulp.lpSum(Y[i, k, j] for k in D_z[z] if k in D_e[i]) == W[i, z, j], f"Zone_Track2_{i}_{z}_{j}"
    
    cohesion_score = pulp.lpSum(cohesion_vars.values())
    
    model2 += cohesion_score
    
    # Restricción de optimalidad de etapa 1
    model2 += pulp.lpSum(X[i, j] for i in E for j in T if j in T_e.get(i, T)) >= optimal_preference - 0.5, "Maintain_Stage1_Optimality"
    
    # Resolver etapa 2
    model2.solve(solver)
    
    if verbose:
        print(f"Estado etapa 2: {pulp.LpStatus[model2.status]}")
        print(f"Valor objetivo etapa 2 (grupos cohesionados): {pulp.value(model2.objective)}")
    
    # Extraer solución final
    solution = {
        'status': pulp.LpStatus[model2.status],
        'stage1_value': optimal_preference,
        'stage2_value': pulp.value(model2.objective),
    ...
    # Zonas de equipos cohesionados
    # for g in G:
    #     solution['team_zones'][g] = []
    #     for z in Z:
    #         if pulp.value(C[g, z]) == 1:
    #             solution['team_zones'][g].append(z)
